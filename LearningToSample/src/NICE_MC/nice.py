"""Classes for creating the NICE layers used inside the nice sampler"""

import tensorflow as tf
from tensorflow.layers import dense

from LearningToSample.src.HMC.hmc import hamiltonian, metropolis_hastings_accept
    ...
    def add(self, x, dx, reuse=False):
        with tf.variable_scope(self.name, reuse=reuse):
            for dim in self.dims:
                x = dense(x, dim, activation=tf.nn.relu)
            x = dense(x, dx)
            return x
