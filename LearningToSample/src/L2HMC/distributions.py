...
def quadratic_gaussian(x, mu, S):
    return tf.diag_part(
        0.5 * tf.matmul(tf.matmul(x - mu, S), tf.transpose(x - mu)))
...
class TiltedGaussian(Gaussian):
    def __init__(self, dim, log_min, log_max):
        ...
        self.diag = np.diag(np.exp(
            np.log(10.) * np.random.uniform(log_min, log_max,
                                            size=(dim,)))) + 1e-8 * np.eye(dim)
...
class GMM(object):
    def __init__(self, mus, sigmas, pis):
        ...
        for i, sigma in enumerate(sigmas):
            ...
            det = np.sqrt((2 * np.pi) ** self.k * np.linalg.det(sigma)).astype(
                'float32')
    ...
    def get_samples(self, n):
        ...
        for k, v in counter_samples.items():
            samples.append(
                np.random.multivariate_normal(self.mus[k], self.sigmas[k],
                                              size=(v,)))
...
class GaussianFunnel(object):
    ...
    def get_energy_function(self):
        ...
        def fn(x):
            ...
            E_safe1 = 0.5 * (log_p_v + sum_sq / s_max + n * tf.log(
                2.0 * np.pi * s_max))
            E_safe2 = 0.5 * (log_p_v + sum_sq / s_min + n * tf.log(
                2.0 * np.pi * s_min))
