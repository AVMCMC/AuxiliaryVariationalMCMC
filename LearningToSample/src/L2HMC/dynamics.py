...
class Dynamics(object):
    ...
    def _forward_step(self, x, v, step, aux=None):
        with tf.name_scope('forward_step'):
            ...
            v_h = tf.multiply(v, safe_exp(sv1, name='sv1F')) + 0.5 * self.eps * (
                        -tf.multiply(safe_exp(fv1, name='fv1F'), grad1) + tv1)
            ...
            y = m * x + mb * (
                        tf.multiply(x, safe_exp(sx1, name='sx1F')) + self.eps * (
                            tf.multiply(safe_exp(fx1, name='fx1F'), v_h) + tx1))
            ...
            x_o = mb * y + m * (
                        tf.multiply(y, safe_exp(sx2, name='sx2F')) + self.eps * (
                            tf.multiply(safe_exp(fx2, name='fx2F'), v_h) + tx2))
            ...
            v_o = tf.multiply(v_h, safe_exp(sv2, name='sv2F')) + 0.5 * self.eps * (
                        -tf.multiply(safe_exp(fv2, name='fv2F'), grad2) + tv2)
    ...
    def _backward_step(self, x_o, v_o, step, aux=None):
        with tf.name_scope('backward_step'):
            ...
            v_h = tf.multiply((v_o - 0.5 * self.eps * (
                        -tf.multiply(safe_exp(fv2, name='fv2B'), grad1) + tv2)),
                              safe_exp(sv2, name='sv2B'))
            ...
            y = mb * x_o + m * tf.multiply(safe_exp(sx2, name='sx2B'), (
                        x_o - self.eps * (
                            tf.multiply(safe_exp(fx2, name='fx2B'), v_h) + tx2)))
            ...
            x = m * y + mb * tf.multiply(safe_exp(sx1, name='sx1B'), (
                        y - self.eps * (
                            tf.multiply(safe_exp(fx1, name='fx1B'), v_h) + tx1)))
            ...
            v = tf.multiply(safe_exp(sv1, name='sv1B'), (v_h - 0.5 * self.eps * (
                        -tf.multiply(safe_exp(fv1, name='fv1B'), grad2) + tv1)))
