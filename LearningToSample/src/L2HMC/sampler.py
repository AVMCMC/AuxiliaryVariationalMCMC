...
def propose(x, dynamics, init_v=None, aux=None,
    ...
    if dynamics.hmc:
        ...
        px = tf.squeeze(mask, axis=1) * px1 + tf.squeeze(1 - mask,
                                                         axis=1) * px2
...
def chain_operator(init_x, dynamics, nb_steps, aux=None, init_v=None,
                   do_mh_step=False):
    if not init_v:
    ...
    def body(x, v, log_jac, t):
        Lx, Lv, px, _ = propose(x, dynamics, init_v=v, aux=aux, log_jac=True,
                                do_mh_step=False)
    ...
    p_accept = dynamics.p_accept(init_x, init_v, final_x, final_v, log_jac,
                                 aux=aux)
