    ...
    def __init__(self, in_, out_, scope='linear', factor=1.0):
        with tf.variable_scope(scope):
            initializer = tf.contrib.layers.variance_scaling_initializer(
                factor=factor * 2.0, mode='FAN_IN', uniform=False,
                dtype=TF_FLOAT)
            self.W = tf.get_variable('W', shape=(in_, out_),
                                     initializer=initializer)
            self.b = tf.get_variable('b', shape=(out_,),
    ...
    def __init__(self, ins_, out_, factors=None, scope='concat_linear'):
        ...
        with tf.variable_scope(scope):
            for i, in_ in enumerate(ins_):
                ...
                self.layers.append(
                    Linear(in_, out_, scope='linear_%d' % i, factor=factor))
...
class ScaleTanh(object):
    def __init__(self, in_, scope='scale_tanh'):
        with tf.variable_scope(scope):
            self.scale = tf.exp(tf.get_variable('scale', shape=(1, in_),
